...
var nicEditorConfig = bkClass.extend({
	...
	buttonList : ['bold','italic','underline','left','center','right','justify','ol','ul','indent','outdent','image','upload','link','unlink',"pcode","youTube"],
	iconList : {"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"removeformat":13,"right":14,"save":25,"strikethrough":16,"subscript":17,"superscript":18,"ul":19,"underline":20,"image":21,"link":22,"unlink":23,"close":24,"arrow":26,"upload":27,'youTube':3,'pcode':25}
...
var nicEditorInstance = bkClass.extend({
    ...
    construct : function(e,options,nicEditor) {
        ...
        if(isTextarea || this.options.hasPanel) {
            ...
            var s = {width: newX+'px',background: 'white' , border : '1px solid #ccc9d1', borderTop : 0, overflowY : 'auto', overflowX: 'hidden' };
...
var nicLinkButton=nicEditorAdvancedButton.extend({addPane:function(){this.ln=this.ne.selectedInstance.selElm().parentTag("A");this.addForm({"":{type:"title",txt:"Add/Edit Link"},href:{type:"text",txt:"URL",value:"http://",style:{width:"150px",height:"30px"}},title:{type:"text",txt:"Title"},target:{type:"select",txt:"Open In",options:{"":"Current Window",_blank:"New Window"},style:{width:"100px"}}},this.ln)},submit:function(C){var A=this.inputs.href.value;if(A=="http://"||A==""){alert("You must enter a URL to Create a Link");return false}this.removePane();if(!this.ln){var B="javascript:nicTemp();";this.ne.nicCommand("createlink",B);this.ln=this.findElm("A","href",B)}if(this.ln){this.ln.setAttributes({href:this.inputs.href.value,title:this.inputs.title.value,target:this.inputs.target.options[this.inputs.target.selectedIndex].value})}}});nicEditors.registerPlugin(nicPlugin,nicLinkOptions);
...
var nicImageButton=nicEditorAdvancedButton.extend({addPane:function(){this.im=this.ne.selectedInstance.selElm().parentTag("IMG");this.addForm({"":{type:"title",txt:"Add/Edit Image"},src:{type:"text",txt:"URL",value:"http://",style:{width:"150px",height:"30px"}},alt:{type:"text",txt:"Alt Text",style:{width:"100px",height:"30px"}},align:{type:"select",txt:"Align",options:{none:"Default",left:"Left",right:"Right"}}},this.im)},submit:function(B){var C=this.inputs.src.value;if(C==""||C=="http://"){alert("You must enter a Image URL to insert");return false}this.removePane();if(!this.im){var A="javascript:nicImTemp();";this.ne.nicCommand("insertImage",A);this.im=this.findElm("IMG","src",A)}if(this.im){this.im.setAttributes({src:this.inputs.src.value,alt:this.inputs.alt.value,align:this.inputs.align.value})}}});nicEditors.registerPlugin(nicPlugin,nicImageOptions);
...
var nicCodeOptions = {
	buttons : {
		'pcode' : {name : 'paste Code', type : 'nicPcodeButton'},
    'youTube' : {name : 'paste YouTube Url', type : 'nicYouTubeButton'}
	}
...
var nicCodeButton=nicEditorAdvancedButton.extend({
	width:"350px",
	addPane:function(){
		this.addForm({"":{type:"title",txt:"Edit HTML"},code:{type:"content",value:this.ne.selectedInstance.getContent(),style:{width:"340px",height:"200px"}}})},submit:function(B){var A=this.inputs.code.value;
...
addPane: function () {
    this.addForm({
        ...
        'youTubeUrl': { type: 'text', txt: 'URL', value: '', style: { width: '150px', height: 'auto'} },
...
submit: function (e) {
    ...
    var downloadYoutube = code.replace('youtube','ssyoutube');

    //var youTubeCode = '<b> <a href="'+downloadYoutube+'"> Download Here </a> </b>  <br/>  <iframe width="' + width + '" height="' + height + '" src="' + code + '" frameborder="0" allowfullscreen></iframe>';
    var youTubeCode = '<br/>  <iframe width="' + width + '" height="' + height + '" src="' + code + '" frameborder="0" allowfullscreen></iframe>';

    this.removePane();
    this.ne.nicCommand('insertHTML', youTubeCode);
}
});
var nicPcodeButton = nicEditorAdvancedButton.extend({
width: '350px',
height: '250px',

addPane: function () {
    this.addForm({
        '': { type: 'title', txt: 'Paste raw code' },
        'pastedCode': { type: 'content', txt: '', value: '', style: { width: '340px',height: '200px'} },
    });
},

submit: function (e) {
    var code = this.inputs['pastedCode'].value;
    code = code.replace(/^\s*\n/gm, ""); //remove empty lines
    //convert to htmlentities ie. conver things like < to &lt;
    code = String(code).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
    var code = '<pre>'+code+'</pre> <br/>';
    this.removePane();
